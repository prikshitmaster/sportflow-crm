...
export default function Batches() {
  ...
  // Seats Free — each batch's own capacity minus who's actually enrolled.
  const seatsFree = useMemo(() => visibleBatches.reduce(
    (total, b) => total + Math.max(0, (b.capacity || 0) - (countByBatch[b.id] || 0)), 0
  ), [visibleBatches, countByBatch])
...
function BatchCard({ b, liveCount = 0, staff = [], onSelect, onEdit, canEdit }) {
  ...
  return (
    <div
      className="bg-white rounded-2xl shadow-sm transition-all border relative hover:shadow-lg active:scale-[0.98] border-gray-200">
      ...
      <div className="p-2 pb-0">
        <div
          className="relative rounded-xl px-4 pt-3.5 pb-3.5 cursor-pointer"
...
function BatchDetailPanel({ batch: b, students, staff, canManageBatches, canManageStudents, onClose, onEdit, onDelete, onAssignCoach, onEnrolledChange }) {
  ...
  // The server can refuse an enrolment when the batch is at capacity. Its
  // message names the limit to raise, so it must reach the screen — this
  // handler used to be try/finally with no catch, which turned a refusal
  ...
  const handleAssign = async (student) => {
    ...
    try {
      ...
      // ("Batch full: … Raise this batch's capacity"). Show it verbatim
