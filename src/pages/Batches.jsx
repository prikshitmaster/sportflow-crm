          ...
          return (
            <div key={b.id} className="card p-5 hover:shadow-md transition">
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-700">{b.coach}</p>
...
function AddBatchModal({ onClose, onSave, staff }) {
  const [form, setForm] = useState({
    name: '', time: '', sports: [], coach: staff[0]?.name || '', capacity: 20, enrolled: 0,
  })
    ...
    <Modal title="Create New Batch" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <input className="input" placeholder="e.g. 6:00 AM – 7:30 AM" value={form.time} onChange={e => set('time', e.target.value)} />
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {SPORTS.map(s => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition ${form.sports.includes(s) ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
              >
                {s}
              </button>
            ))}
