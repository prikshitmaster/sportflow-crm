...
BEGIN
  ...
  INSERT INTO trials (
    ...
    0,
    false,
    NULLIF(p_payload->>'followUp', ''),
    NULLIF(p_payload->>'notes', ''),
    NULLIF(p_payload->>'quotedFee', '')::NUMERIC,
    NULLIF(p_payload->>'sessionStart', ''),
    NULLIF(p_payload->>'sessionEnd', ''),
...
BEGIN
  ...
  UPDATE trials SET
    ...
    follow_up      = CASE WHEN p_payload ? 'followUp'      THEN NULLIF(p_payload->>'followUp','')               ELSE follow_up      END,
    ...
    session_start  = CASE WHEN p_payload ? 'sessionStart'  THEN NULLIF(p_payload->>'sessionStart','')           ELSE session_start  END,
    session_end    = CASE WHEN p_payload ? 'sessionEnd'    THEN NULLIF(p_payload->>'sessionEnd','')             ELSE session_end    END,
