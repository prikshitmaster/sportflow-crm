-- 0169 — Remove the /join anti-spam submission cap, for now
--
-- secure_submit_public_trial_v2 refused a 5th registration from the same
-- phone within 24h ("too many submissions"). Hit during today's live demo
-- (registering multiple test students back-to-back under one phone) —
-- removed per explicit request. Purely a business-rule removal, easy to
-- reinstate later (see git history for the exact WHERE/COUNT block) if spam
-- becomes a real problem once this is genuinely public.
--
-- Everything else (auth, branch/academy/batch checks, sibling-link ownership
-- check, phone normalization) is untouched.
--
-- Signature UNCHANGED — plain CREATE OR REPLACE. IDEMPOTENT.
...
BEGIN
  ...
  -- Anti-spam daily-submission cap removed (0169) — see migration header.
