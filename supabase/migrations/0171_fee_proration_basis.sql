-- ============================================================
-- 0171 — Per-branch partial-month proration basis
-- ============================================================
-- WHAT
--   sport_branches.fee_proration_basis — how a partial leading month (a
--   student joining/paying mid-month, via Custom coverage dates in
--   RecordPaymentModal) gets prorated: 'calendar' (days actually in that
--   month — 28-31) or '30day' (fixed 30-day month, flat per-day rate
--   year-round). Academy-configurable per branch, same place as trial/kit
--   fee and tax settings.
--
--   secure_update_branch_fees gains p_proration_basis. Signature changed
--   (new arg), so per the precedent in 0137/security-v3-28, the OLD
...
ALTER TABLE sport_branches
  ADD COLUMN IF NOT EXISTS fee_proration_basis TEXT NOT NULL DEFAULT 'calendar'
  CHECK (fee_proration_basis IN ('calendar', '30day'));

DROP FUNCTION IF EXISTS secure_update_branch_fees(UUID, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, BOOLEAN, BOOLEAN, TEXT);
...
END;
$$;
GRANT EXECUTE ON FUNCTION secure_update_branch_fees(UUID, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT) TO anon, authenticated;
...
-- ============================================================
-- SELECT column_name FROM information_schema.columns WHERE table_name = 'sport_branches' AND column_name = 'fee_proration_basis';
-- SELECT proname, pronargs FROM pg_proc WHERE proname = 'secure_update_branch_fees';
