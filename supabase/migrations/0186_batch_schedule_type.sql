-- 0186 — Batch Schedule Type (Alternate / Daily)
--
-- Batches now carry an explicit schedule-frequency tag, mirroring how 0157
-- added batch_type (Development/Advance). Two values only:
--   'alternate' (default) — a fixed 3ish-day pattern (MWF, TTS, ...)
--   'daily'               — trains every day
--
-- WHY
--   "Alternate vs Daily" previously only existed on the STUDENT
--   (students.training_type). A batch's own "Daily" status was inferred by
--   comparing its days to whatever slot-group it happened to be in
--   (isFullWeekBatch in src/lib/batchCapacity.js) — fragile, since adding a
--   new batch to a group could silently change another batch's label. This
--   makes it an explicit, owner-set property of the batch itself instead.
--
--   Does NOT change how shared-ground seat math works — that still keys off
--   each STUDENT's own training_type, because one batch can hold a mix of
--   Alternate and Daily students. This is a label + smart UI defaults only.
--
-- WHAT CHANGES
--   batches.schedule_type — new column, NOT NULL, defaults to 'alternate',
--                            CHECK-constrained to the two values. Existing
--                            batches with 6+ configured days are backfilled
--                            to 'daily' (they were already behaving as one).
--   secure_insert_batch    — gains p_schedule_type. Old signature DROPPED
--                            first (same reason as 0157: PostgREST must
--                            never see two overloads of the same name).
--   secure_update_batch    — payload-driven, gains one CASE line; signature
--                            unchanged.
--
...
-- ── Column + backfill ─────────────────────────────────────
ALTER TABLE batches
  ADD COLUMN IF NOT EXISTS schedule_type TEXT NOT NULL DEFAULT 'alternate';

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'batches_schedule_type_check'
  ) THEN
    ALTER TABLE batches
      ADD CONSTRAINT batches_schedule_type_check
      CHECK (schedule_type IN ('alternate', 'daily'));
  END IF;
END $$;

-- A batch already configured for 6+ days a week was already behaving as a
-- daily batch — backfill it explicitly so display doesn't regress.
UPDATE batches SET schedule_type = 'daily'
 WHERE array_length(days, 1) >= 6 AND schedule_type <> 'daily';
...
CREATE OR REPLACE FUNCTION secure_insert_batch(
  ...
  p_schedule_type  TEXT       DEFAULT 'alternate'
) RETURNS JSON
...
BEGIN
  ...
  v_sched := lower(COALESCE(NULLIF(trim(p_schedule_type), ''), 'alternate'));
  IF v_sched NOT IN ('alternate', 'daily') THEN
  ...
  INSERT INTO batches (
    ...
    default_fee, default_plan, batch_type, schedule_type, academy_id, branch_id
  ) VALUES (
    p_name,
    p_time,
...
GRANT EXECUTE ON FUNCTION secure_insert_batch(
  TEXT, TEXT, TEXT, JSONB, TEXT, INTEGER, JSONB, TEXT, TEXT,
  INTEGER, INTEGER, TEXT, TEXT, INTEGER, TEXT, UUID, TEXT, TEXT
