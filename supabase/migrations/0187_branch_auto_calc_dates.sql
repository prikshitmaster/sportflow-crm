-- ============================================================
-- 0187 — Per-branch auto-calculate payment dates (on/off)
-- ============================================================
-- WHAT
--   sport_branches.auto_calc_payment_dates — master on/off for date-based
--   auto-calculation across the payments/students flows:
--     ON  (default) — current behaviour: Record Payment auto month-picks
--         due months, Add/Edit Student auto-fills Paid Till from Join Date
--         + Fee Plan. Per-instance manual overrides ("Custom coverage
--         dates" checkbox, "Custom" fee plan) still work as before.
--     OFF — auto-calculation never runs for this branch; every date is
--         entered by hand everywhere, with no per-instance toggle needed
--         since there's nothing to opt out of.
--
--   Lives next to fee_proration_basis (0171) since both are about how
--   payment dates/amounts get worked out automatically, same settings card.
--
--   secure_update_branch_fees gains p_auto_calc_dates. Signature changed
--   (new arg), so per the precedent in 0171, the OLD signature must be
--   dropped first — CREATE OR REPLACE only replaces a function with the
--   exact same argument types.
--
-- IDEMPOTENT — safe to re-run.
-- ============================================================

BEGIN;

ALTER TABLE sport_branches
  ADD COLUMN IF NOT EXISTS auto_calc_payment_dates BOOLEAN NOT NULL DEFAULT true;

DROP FUNCTION IF EXISTS secure_update_branch_fees(UUID, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT);

CREATE OR REPLACE FUNCTION secure_update_branch_fees(
  p_branch_id         UUID,
  p_trial_fee         NUMERIC DEFAULT NULL,
  p_kit_fee           NUMERIC DEFAULT NULL,
  p_tax_percent       NUMERIC DEFAULT NULL,
  p_tax_on_fees       BOOLEAN DEFAULT FALSE,
  p_tax_on_trial      BOOLEAN DEFAULT FALSE,
  p_tax_on_kit        BOOLEAN DEFAULT FALSE,
  p_proration_basis   TEXT    DEFAULT NULL,
  p_auto_calc_dates   BOOLEAN DEFAULT NULL,
  p_token             TEXT    DEFAULT NULL
) RETURNS VOID
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  a       RECORD;
  v_acad  UUID;
  v_mgr   BIGINT;
BEGIN
  SELECT * INTO a FROM current_actor(p_token) LIMIT 1;
  IF a.actor_kind IS NULL THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;

  SELECT academy_id, manager_id INTO v_acad, v_mgr
    FROM sport_branches WHERE id = p_branch_id;

  IF NOT FOUND OR v_acad IS DISTINCT FROM a.academy_id THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
  END IF;

  IF a.actor_kind <> 'owner'
     AND NOT (a.actor_kind = 'staff' AND v_mgr IS NOT NULL AND v_mgr = a.actor_id) THEN
    RAISE EXCEPTION 'forbidden: not the manager of this branch' USING ERRCODE = '42501';
  END IF;

  IF p_tax_percent IS NOT NULL AND (p_tax_percent < 0 OR p_tax_percent > 100) THEN
    RAISE EXCEPTION 'tax rate must be between 0 and 100' USING ERRCODE = '22023';
  END IF;

  IF p_proration_basis IS NOT NULL AND p_proration_basis NOT IN ('calendar', '30day') THEN
    RAISE EXCEPTION 'proration basis must be calendar or 30day' USING ERRCODE = '22023';
  END IF;

  UPDATE sport_branches SET
    trial_fee                = p_trial_fee,
    kit_fee                  = p_kit_fee,
    tax_percent               = p_tax_percent,
    tax_on_fees               = COALESCE(p_tax_on_fees,  FALSE),
    tax_on_trial              = COALESCE(p_tax_on_trial, FALSE),
    tax_on_kit                = COALESCE(p_tax_on_kit,   FALSE),
    fee_proration_basis       = COALESCE(p_proration_basis, fee_proration_basis),
    auto_calc_payment_dates   = COALESCE(p_auto_calc_dates, auto_calc_payment_dates)
  WHERE id = p_branch_id;
END;
$$;
GRANT EXECUTE ON FUNCTION secure_update_branch_fees(UUID, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, BOOLEAN, TEXT) TO anon, authenticated;

COMMIT;
