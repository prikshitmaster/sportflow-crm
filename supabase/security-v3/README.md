...
| Phase 3.4c | **Fix regression from 3.4b**: `notifications`/`push_subscriptions` lost their only-ever policy (an untargeted `FOR ALL`, which defaults to PUBLIC) and got back SELECT-only `TO anon` — silently breaking all notification sending/read-marking/deleting for staff+student, and *all* access (including read) for the owner. Restores anon INSERT/UPDATE/DELETE (academy-scoped, matching the existing anon_read) and an authenticated-role policy (matching the `announcements_auth_all` pattern used elsewhere). | `13_fix_notifications_write_policies.sql` | ✅ applied 2026-07-27 |
